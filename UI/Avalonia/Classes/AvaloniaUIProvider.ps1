#ImportOrder 60
#
# Avalonia concrete subclass of UIProvider. Each method captures the
# corresponding free function from CoreUIAvalonia.ps1 (and friends) at construction
...
#
# Folder picking is exposed through the provider even though the implementation
# delegates to Avalonia's StorageProvider host wrapper.

class AvaloniaUIProvider : UIProvider {
    ...
    [string] ShowFolderPicker([string]$Description) {
        $f = ${function:Get-AvaloniaHost}
        $hostType = & $f
        return $hostType::OpenFolderPicker($script:Window, $Description)
    }

    [string] ShowFolderPicker([string]$Path, [string]$Description) {
        $f = ${function:Get-AvaloniaHost}
        $hostType = & $f
        return $hostType::OpenFolderPicker($script:Window, $Description, $Path)
    }
    ...
    [void] SetControlVisible([object]$XamlObj, [string]$ControlName, [bool]$Visible) {
        $this.SetXamlProperty($XamlObj, $ControlName, "IsVisible", $Visible)
    }
    ...
    [void] SetClipboardText([string]$Text) {
        $f = ${function:Get-AvaloniaHost}
        $hostType = & $f
        $hostType::SetClipboardText($script:Window, $Text)
    }
